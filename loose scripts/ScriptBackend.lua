...
function ScriptBackend.toggleScript(scr,value)
	if not scr or not scr.script or scr.enabled == value then return end
...
pcall(function()
	...
	CommandPalette.commands.script = {
		...
		execute = function(self,sep,all)
			...
			ScriptBackend.pingScripts()
			return ('{"text":"Set state of %s to %s!","color":"green"}'):format(script.name,script.enabled and "visible" or "invisible")
		end
